...
def preProcessing(df,seed,trainPerc):
    ...
    training = df.sample(frac=trainPerc,random_state=seed)
    
    test = df.drop(training.index)
##    size = df.shape[0]
##    indexList = list(range(size))
##    trainingLimit = round(trainPerc * size)
...
def matrix(actual,predicted,labels,approach,dataSet): #takes results from algorithms and creates a csv file matrix
    ...
    for p in range(len(predicted)): #filling in the values for the matrix
        #print(p)
        #print("actual",actual[p])
        #print("predicted",predicted[p])
        #print("labels",labels)
