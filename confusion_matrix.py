def buildConfusionMatrix(p, labels, numLabels, threshold):
    if numLabels == 1:
        return build2DConfusionMatrix(p, labels, threshold)
    else:
        return buildNDConfusionMatrix(p, labels, numLabels)


def build2DConfusionMatrix(p, labels, threshold):
    preds = []
    ...
    for i in range(len(preds)):
        #print(i, labels[i], len(labels), len(preds))
        actual = labels[i][0]
        predicted = preds[i]
